...
boolean Obj_Combo_Tick(Object *obj)
{
	...
	if (this->hit_type != 0xFF && this->ht < (FIXED_DEC(16,1) / 60))
	{
		...
		Stage_DrawTex(&stage.tex_hud0p, &hit_src, &hit_dst, stage.camera.bzoom);
	...
	if (this->num[4] != 0xFF && this->ct < (FIXED_DEC(16,1) / 60))
	{
		...
		Stage_DrawTex(&stage.tex_hud0p, &combo_src, &combo_dst, stage.camera.bzoom);
	...
	if (this->numt < (FIXED_DEC(16,1) / 60))
	{
		for (u8 i = 0; i < 5; i++)
		{
			...
			Stage_DrawTex(&stage.tex_hud0p, &num_src, &num_dst, stage.camera.bzoom);
