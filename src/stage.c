...
#include "character/bfmii.h"
#include "character/gfmii.h"
#include "character/eteleda.h"
#include "character/eteledb.h"
#include "character/eteledc.h"
#include "character/banan.h"
#include "character/austine.h"
#include "character/austinp.h"

#include "stage/dummy.h"
#include "stage/plaza.h"
#include "stage/plazab.h"
#include "stage/hall.h"
#include "stage/plazac.h"
			...
			if (note->type & NOTE_FLAG_SUSTAIN && note->type & NOTE_FLAG_OPPONENT)
			{
				...
				if (note->type & NOTE_FLAG_SUSTAIN_END && note->type & NOTE_FLAG_OPPONENT)
				{
					if (clip < (24 << FIXED_SHIFT))
					{
						...
						Stage_DrawTex(&stage.tex_hud0e, &note_src, &note_dst, stage.bump);
					}
				}
				else
				{
...
void Stage_Load(StageId id, StageDiff difficulty, boolean story)
{
	...
	//Load HUD textures
	//yandere simulator code oh yeeeaaaah
    if (stage.mode == StageMode_Swap)
	    {
	        if (id == StageId_1_1)
	        	Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0BF.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0DEL.TIM;1"), GFX_LOADTEX_FREE);
	        else if (id == StageId_1_2)
	            Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0BF.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0DEL.TIM;1"), GFX_LOADTEX_FREE);
			else if (id == StageId_1_3)
	            Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0BF.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0AUST.TIM;1"), GFX_LOADTEX_FREE);
			else if (id == StageId_1_4)
	            Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0AUSP.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0DEL.TIM;1"), GFX_LOADTEX_FREE);
			else if (id == StageId_1_5)
	            Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0BF.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0DEL.TIM;1"), GFX_LOADTEX_FREE);
			else if (id == StageId_1_6)
	            Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0BLNK.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0BLNK.TIM;1"), GFX_LOADTEX_FREE);
	        else
	        	Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0.TIM;1"), GFX_LOADTEX_FREE);
	    ...
	    {
			 if (id == StageId_1_1)
	        	Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0DEL.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0BF.TIM;1"), GFX_LOADTEX_FREE);
	        else if (id == StageId_1_2)
	            Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0DEL.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0BF.TIM;1"), GFX_LOADTEX_FREE);
			else if (id == StageId_1_3)
	            Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0AUST.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0BF.TIM;1"), GFX_LOADTEX_FREE);
			else if (id == StageId_1_4)
	            Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0DEL.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0AUSP.TIM;1"), GFX_LOADTEX_FREE);
			else if (id == StageId_1_5)
	            Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0DEL.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0BF.TIM;1"), GFX_LOADTEX_FREE);
			else if (id == StageId_1_6)
	            Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0BLNK.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0BLNK.TIM;1"), GFX_LOADTEX_FREE);
	...
	switch (stage.state)
	{
		case StageState_Play:
		{
			...
			//trackers
			  ...
			  //FntPrint("noise is %d ", stage.noise);
			  ...
			  else if (stage.stage_id == StageId_1_5)
			  {
			  stage.healthe = 16;
			  stage.healthp = 8;
			  }
			...
			if (playing && (stage.flag & STAGE_FLAG_JUST_STEP))
			{
				...
				//M.I.L.F bumps
				//if (stage.stage_id == StageId_2_3 && stage.song_step >= (168 << 2) && stage.song_step < (200 << 2))
			...
			for (int i = 0; i < ((stage.mode >= StageMode_2P) ? 2 : 1); i++)
			{
				...
				Stage_DrawTex(&stage.tex_hud0p, &score_src, &score_dst, stage.bump);
